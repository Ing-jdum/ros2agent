...
namespace plansys2_terminal
{
  ...
  Terminal::init()
  {
    ...
    server_ = std::make_unique<TCPServer>(8080);
    add_problem();
  ...
  Terminal::run_console()
  {
    ...
    std::cout << " Hola from ROS2 Planning System console. Type \"quit\" to finish" << std::endl;
    ...
    while (!finish_program)
    {
      ...
      bool finish_message = false;
      while (!finish_message)
      {
        char *input_line = server_->receiveMessage();
  ...
  Terminal::process_get_model_predicate(std::vector<std::string> &command, std::ostringstream &os)
  {
    if (command.size() == 1)
    {
      ...
      if (predicates)
      {
        ...
        {
          os << "\t" << predicates.value().parameters[i].type << " - " << predicates.value().parameters[i].name << std::endl;
        }
      }
      else
      {
  ...
  Terminal::process_get_model_function(std::vector<std::string> &command, std::ostringstream &os)
  {
    if (command.size() == 1)
    {
      ...
      if (functions)
      {
        ...
        {
          os << "\t" << functions.value().parameters[i].type << " - " << functions.value().parameters[i].name << std::endl;
        }
      }
      else
      {
  ...
  Terminal::process_get_model_action(std::vector<std::string> &command, std::ostringstream &os)
  {
    if (command.size() == 1)
    {
      ...
      if (action)
      {
        ...
        {
          os << "\t" << action->parameters[i].type << " - " << action->parameters[i].name << std::endl;
        }
      ...
      else if (durative_action)
      {
        ...
        {
          os << "\t" << durative_action->parameters[i].name << " - " << durative_action->parameters[i].type << std::endl;
        }
        os << "AtStart requirements: " << parser::pddl::toString(durative_action->at_start_requirements) << std::endl;
        os << "OverAll requirements: " << parser::pddl::toString(durative_action->over_all_requirements) << std::endl;
        os << "AtEnd requirements: " << parser::pddl::toString(durative_action->at_end_requirements) << std::endl;
        os << "AtStart effect: " << parser::pddl::toString(durative_action->at_start_effects) << std::endl;
        os << "AtEnd effect: " << parser::pddl::toString(durative_action->at_end_effects) << std::endl;
      }
      else
      {
        os << "Error when looking for params of " << command[0] << std::endl;
    ...
    if (command.size() > 0)
    {
      ...
      else if (command[0] == "plan")
      {
        ...
        if (plan)
        {
          ...
          {
            os << plan_item.time << ":\t" << plan_item.action << "\t[" << plan_item.duration << "]" << std::endl;
          }
        }
        else
        {
  ...
  Terminal::process_set_predicate(std::vector<std::string> &command, std::ostringstream &os)
  {
    if (command.size() > 0)
    {
      ...
      {
        os << "Could not add the predicate [" << parser::pddl::toString(predicate) << "]" << std::endl;
      }
    }
    else
    {
  ...
  Terminal::process_remove_predicate(std::vector<std::string> &command, std::ostringstream &os)
  {
    ...
    if (command.size() > 0)
    {
      ...
      {
        os << "Could not remove the predicate [" << parser::pddl::toString(predicate) << "]" << std::endl;
      }
    }
    else
    {
  ...
  Terminal::process_remove_function(std::vector<std::string> &command, std::ostringstream &os)
  {
    if (command.size() > 0)
    {
      ...
      {
        os << "Could not remove the function [" << parser::pddl::toString(function) << "]" << std::endl;
      }
    }
    else
    {
      os << "\tUsage: \n\t\remove function [name] [instance1] [instance2] ...[instaceN]" << std::endl;
    }
  }
...
void Terminal::execute_plan(int items)
{
    ...
    std::string message; // Use an std::string for output messages

    if (!plan.has_value())
    {
        message = "Plan could not be computed";
        std::cout << message << std::endl; // Output the message to standard output
        server_->sendMessage(message); // Send the message through server_
        return;
    }
    ...
    {
        message = "Can't execute " + std::to_string(items) + " actions";
        std::cout << message << std::endl; // Output the message to standard output
        server_->sendMessage(message); // Send the message through server_
        return;
    }
  ...
  Terminal::execute_plan(const plansys2_msgs::msg::Plan &plan)
  {
    ...
    std::string message; // Use std::string for output messages
  ...
  Terminal::process_run(std::vector<std::string> &command, std::ostringstream &os)
  {
    ...
    else
    {
      ...
      {
        os << "\tUsage: \n\t\trun" << std::endl;
        os << "\tUsage: \n\t\trun num_actions [number of actions to execute from plan]" << std::endl;
