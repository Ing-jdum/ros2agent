...
namespace plansys2_terminal
{
  ...
  Terminal::Terminal()
    : rclcpp::Node("terminal"), server_(std::make_unique<TCPServer>(8080))  // Initialize TCPServer
  {
...
Terminal::process_set_goal(std::vector<std::string> &command, std::ostringstream &os)
{
  if (command.size() > 0)
  {
    ...
    {
      os << "Error parsing goal: Invalid expression provided." << std::endl;
      os << "Details: " << e.what() << std::endl;
    }
    catch (const std::exception &e)
    {
      os << "An unexpected error occurred: " << e.what() << std::endl;
    }
  }
  else
  {
  ...
  Terminal::execute_plan(const plansys2_msgs::msg::Plan &plan)
  {
    ...
    if (executor_client_->getResult().value().success)
    {
      ...
      server_->sendMessage("Successful finished ");
    }
    else
    {
      std::cout << "Finished with error(s) " << std::endl;
      server_->sendMessage("Finished with error(s)");
    }
  }
